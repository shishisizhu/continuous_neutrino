...
#define RING_BUFFER_SIZE  (1 << 18)  // 65536 entries (power of 2)
...
int log_write(const char *msg) {
    ...
    if (next_head == tail) {
        fprintf(stderr, "log_write: Ring buffer is full, message discarded.\n");
...
static void* log_consumer_thread(void *arg) {
    ...
    while (1) {
        ...
        do {
            ...
            fprintf(fp, "[%lu.%09lu] TID-%u: %s", sec, ns, entry->tid, entry->message);
