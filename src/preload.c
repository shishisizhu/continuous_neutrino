...
/**
 * @bug this shall be auto-detected and build
 * @fix add this to parse.py of our building system
 */
#ifndef REAL_LIB_NAME
#define REAL_LIB_NAME "libcuda.so.550.54.15"
#endif
...
void* dlopen(const char *filename, int flags) {
    ...
    if (filename != NULL && (strstr(filename, "libcuda.so.1") != NULL)) {
        ...
        return ptr;
    } else if (filename != NULL && strstr(filename, REAL_LIB_NAME) != NULL) {
        // Check if it's libcublas.so
        void* array[STACK_TRACE_SIZE];
        // Use backtrace to check if it's cublas loading libcuda.so.1
        int size       = backtrace(array, STACK_TRACE_SIZE);
        char** strings = backtrace_symbols (array, size);
        int call_from_cublas = 0;
        if (strings != NULL){
